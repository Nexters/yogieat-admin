...
const PAGE_SIZE = 8;
const DEFAULT_REGION_OPTIONS = ["ALL", "HONGDAE", "GANGNAM", "GONGDEOK"];
...
export function GatheringListPage() {
	...
	const [pageResponse, setPageResponse] =
		useState<PageResponse<GatheringListItem>>(DEFAULT_PAGE);
	...
	const fetchGatherings = useCallback(
		async (nextQuery: GatheringListQuery) => {
			setIsLoading(true);
			setErrorMessage("");
			try {
				const response = await adminService.getGatherings(nextQuery);
				setPageResponse(response);
			} catch (error) {
				if (error instanceof Error) {
					setErrorMessage(error.message);
				} else {
					setErrorMessage("모임 목록을 불러오지 못했습니다.");
				}
			} finally {
				setIsLoading(false);
			}
	...
	const handleTimeSlotChange = (timeSlot: string) => {
		setQuery((current) => ({
			...current,
			page: 0,
			timeSlot: timeSlot === "ALL" ? undefined : timeSlot,
		}));
	};
			...
			<header className="admin-topbar">
				<div className="admin-topbar__title-wrap">
					...
					<p className="admin-topbar__mode">
						생성된 모임과 참여 인원 현황을 확인합니다.
					</p>
				</div>
				<div className="admin-topbar__actions admin-topbar__actions--gathering">
						...
						pageResponse.content.map((gathering) => (
							...
							>
								...
								<div className="admin-gathering-card__meta">
									...
									<span>지역: {gathering.region}</span>
									<span>시간대: {gathering.timeSlot}</span>
