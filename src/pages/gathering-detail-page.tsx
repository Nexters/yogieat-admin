...
import { GatheringDetail, adminService } from "../apis/admin";
...
export function GatheringDetailPage() {
	...
	const fetchDetail = useCallback(async () => {
		...
		setErrorMessage("");
		try {
			const response = await adminService.getGatheringById(gatheringId);
			setDetail(response);
		} catch (error) {
			if (error instanceof Error) {
				setErrorMessage(error.message);
			} else {
				setErrorMessage("모임 상세 정보를 불러오지 못했습니다.");
			}
		} finally {
			setIsLoading(false);
		}
	}, [gatheringId, hasValidId]);
		...
		<main className="admin-shell">
			<header className="admin-topbar">
				<div className="admin-topbar__title-wrap">
					<p className="admin-topbar__eyebrow">Gathering Detail</p>
					<h1>{resolveGatheringTitle(detail, gatheringId)}</h1>
					<p className="admin-topbar__mode">
						참여자 {detail.participantCount}명 / 정원{" "}
						{detail.gathering.peopleCount}명
					</p>
				</div>
			...
			<section className="admin-panel admin-panel--detail">
				...
				<div className="admin-detail-grid">
					...
					<label className="admin-field">
						<span>지역</span>
						<div className="admin-readonly">
							{detail.gathering.region}
						</div>
					</label>
					<label className="admin-field">
						<span>시간대</span>
						<div className="admin-readonly">
							{detail.gathering.timeSlot}
						</div>
					</label>
					<label className="admin-field">
						<span>정원</span>
